...
# Agent definition
# tell it to use its own memory so it never asks for the same thing
# define school stat agent as a agent tool so college agent can use it 
# to get school stats
CollegeAgent = Agent(
    name="College_agent",
    model=Gemini(
    ...
    description= "An agent that tells you your chances at a college",
    instruction="""
                ...
                7. End conversation only when user shows intention to leave, to do this, say goodbye or something
                   similer and add a final token: 
